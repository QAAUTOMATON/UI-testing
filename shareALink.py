...
yt_link = "https://www.youtube.com/watch?v=pAh83khT1no"
...
try:
    ...
    if ele.is_displayed():
        ...
        ele2 = driver.find_element(By.XPATH, '//*[@id="navbarSupportedContent"]/ul/li[9]/div/div/a[2]')
        wait(2)
...
current_row = 8
try:
    ele = driver.find_element(By.ID, 'instruction_id_link')
    if ele.is_displayed():
        ele.click()
...
current_row = 10
try:
    ele = driver.find_element(By.ID, 'submit_form')
    if ele.is_displayed():
        ele.click()
        print(current_row)
        writeData(inputFileName, inputSheetName, r + current_row, c + current_column, 'pass')
...
try:
    ...
    if ele.is_displayed():
        ...
        print(current_row)
        ele.click()
        ele.send_keys(f"{reason} this is the test line")
        print(current_row)
        ele2 = driver.find_element(By.ID, 'submit_form')
        ele2.click()
        writeData(inputFileName, inputSheetName, r + current_row, c + current_column, 'pass')
    else:
        writeData(inputFileName, inputSheetName, r + current_row, c + current_column, 'fail')
