...
inputSheetName = 'convertYourScript'
...
doc_path = "C://Users/kz/OneDrive/Desktop/Automation/drive_backup/sampleupload files/ThePremonition.docx"
day = date.today()
# ==========================================================================================================
...
def word_limit_test(element, word_limit):
    ...
    if len(element.get_attribute('value')) == word_limit:
        flag = 1
    else:
        flag = 0
    return flag
...
# ele.click()
# wait(2)
# ele = driver.find_element(By.XPATH, "//a[contains(text(),'Convert Your Script')]")
# ele.click()
...
try:
    ...
    if ele.is_displayed():
        ...
        ele2 = driver.find_element(By.XPATH, "//a[contains(text(),'Convert Your Script')]")
        wait(2)
...
current_row = 6
try:
    ele = driver.find_element(By.ID, 'con_file')
    if ele.is_displayed():
        ele.send_keys(doc_path)
        writeData(inputFileName, inputSheetName, r + current_row + 1, c + current_column, 'pass')
    else:
        writeData(inputFileName, inputSheetName, r + current_row, c + current_column, 'fail')
except:
...
current_row = 8
try:
    ele = driver.find_element(By.XPATH, '//*[@id="import"]/div[2]/div/button')
    if ele.is_displayed():
        ele.click()
        wait(20)
        ele2 = driver.find_element(By.ID, "language4")
        wait(2)
        if ele2.is_displayed():
            writeData(inputFileName, inputSheetName, r + current_row, c + current_column, 'pass')
    else:
...
try:
    for i in range(1, 5):
        ele = driver.find_element(By.XPATH, f'//tbody/tr[{i}]/td[3]/a[1]')
        if ele.is_displayed():
            ele.click()
            wait(5)
            driver.back()
...
# current_row = 14
# try:
#     ele = driver.find_element(By.ID, 'language4')
#     ele2 = driver.find_element(By.ID, 'language6')
#     ele3 = driver.find_element(By.ID, "opt6")
#     counter = 0
#     for i in range(0, 80):
#         wait(.01)
#         Select(ele).select_by_index(i+1)
#         for j in range(0, 80):
#             wait(.01)
#             Select(ele2).select_by_index(j+1)
#             print(f"icount :: jcount : {i} :: {j} ")
#             counter += 1
